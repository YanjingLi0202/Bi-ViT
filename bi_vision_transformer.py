...
import numpy as np
from Binary_plus import *
from _binary_base_plus import *
from Bi_Quant import *
from lsq_plus import *
from _quan_base_plus import *
# import BiLinear as linear # BiLinearMCN_2d as BiLinearMCN
#rom binactivation import *
...
'''class LearnableBias(nn.Module):
    ...
    def forward(self, x):
        if self.bias is None:
            if len(x.shape)==3:
                self.bias=nn.Parameter(torch.zeros(1,1,self.out_chn), requires_grad=True).to(x.device)
            elif len(x.shape)==2:
                self.bias=nn.Parameter(torch.zeros(1,self.out_chn), requires_grad=True).to(x.device)
...
class bi_Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = BiLinearBiReal(in_features, hidden_features)
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        
        # self.fc2 = LinearBi(hidden_features, out_features, mode=Qmodes.kernel_wise)
        self.fc2 = BiLinearBiReal(hidden_features, out_features)
...
class bi_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, quantize_attn=True, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = BiLinearBiReal(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = BiLinearBiReal(dim, dim)
...
class bi_PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = Conv2dBi(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
...
class twobits_VisionTransformer(nn.Module):
    """ Vision Transformer
...
@register_model
def bi_deit_tiny_patch16_224(pretrained=False, **kwargs):
    # model = twobits_VisionTransformer(
    ...
    if True:
        print('!!!!!!!!!!!!!!!!LOAD!!!!!!!!!!!!!!!!!!!!')
        # checkpoint = torch.load('checkpoints/deit_tiny_patch16_224-a1311bcf.pth', map_location='cpu')
        checkpoint = torch.load('checkpoints/deit_tiny_distilled_patch16_224-b40b3cf7.pth', map_location='cpu')
        # checkpoint = torch.load('dist_bi_W1A32_tiny_qknorm2_5e-4_300_512/best_checkpoint.pth', map_location='cpu')
        # checkpoint = torch.load('distill_head_dist_tiny_attn_w_32_8_qknorm2_2e-4_50_2bit_1024/best_checkpoint.pth', map_location='cpu')
        model.load_state_dict(checkpoint["model"], strict=False)
    return model

@register_model
def bi_deit_small_patch16_224(pretrained=False, **kwargs):
    model = twobits_VisionTransformer(
