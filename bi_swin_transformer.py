...
from bi_vision_transformer import bi_Mlp
...
class Bi_WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        # self.v_act = ActLSQ(in_features=self.num_heads, nbits_a=2)
        # self.attn_act = ActLSQ(in_features=self.num_heads, nbits_a=2)
...
class Bi_SwinTransformerBlock(nn.Module):
...
class Bi_PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = LinearBi(4 * dim, 2 * dim, bias=False, mode=Qmodes.kernel_wise)
...
class Bi_SwinTransformer(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            layers += [Bi_BasicLayer(
                ...
                downsample=Bi_PatchMerging if (i_layer < self.num_layers - 1) else None,
...
@register_model
def bi_swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    model = Bi_SwinTransformer(
